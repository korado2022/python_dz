# Создать собственный класс-исключение, который должен проверять содержимое списка
...
RE_DIG = re.compile(r'[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?')
...
while True:
    try:
        ...
        if input_txt == 'stop':
            print(res)
            break
        if not ValueError.check_digit(input_txt):
            raise ValueError('Вы ввели не число')
    except ValueError as err:
        print(err)
    else:
        if '.' in input_txt:
            res.append(float(input_txt))
        else:
            res.append(int(input_txt))
